...
function Invoke-CurlRequest {
    ...
    $headerFile = Join-Path $env:TEMP ("pl-list-smoke-h-" + [guid]::NewGuid().ToString() + ".txt")
    $bodyFile = Join-Path $env:TEMP ("pl-list-smoke-b-" + [guid]::NewGuid().ToString() + ".txt")
    $jsonFile = $null
    ...
    try {
        ...
        if ($null -ne $JsonBody) {
            $jsonFile = Join-Path $env:TEMP ("pl-list-smoke-j-" + [guid]::NewGuid().ToString() + ".json")
...
Write-Host "Private leagues list endpoint smoke checks for TASK-023"
...
if (-not $token) {
    $runEmail = ("pl_list_smoke_" + [int][double]::Parse((Get-Date -UFormat %s)) + "@example.com")
...
Write-Host "3) GET /leagues/$leagueId/private-leagues -> expect 200 + Category A + invites[] + leagues[]"
$list1 = Invoke-CurlRequest -Method GET -Url "$BaseUrl/leagues/$leagueId/private-leagues" -Headers @("Authorization: Bearer $token")
...
if ($list1.status -eq 200) {
    try {
        ...
        $okShape = ($obj.data.invites -is [array]) -and ($obj.data.leagues -is [array]) -and ([int]$obj.data.league_id -eq $leagueId)
...
if ($list1.status -eq 200 -and $cc1 -eq "private, must-revalidate" -and $etag1 -and $okShape -and $okMetaEtag) {
    Write-Host "PASS: private leagues list returned expected envelope and cache headers."
} else {
    Write-Host "FAIL: private leagues list did not meet expected response/header requirements."
}
...
if ($etag1) {
    ...
    $list304 = Invoke-CurlRequest -Method GET -Url "$BaseUrl/leagues/$leagueId/private-leagues" -Headers @(
...
$invalid = Invoke-CurlRequest -Method GET -Url "$BaseUrl/leagues/$InvalidLeagueId/private-leagues" -Headers @("Authorization: Bearer $token")
...
$noToken = Invoke-CurlRequest -Method GET -Url "$BaseUrl/leagues/$leagueId/private-leagues"
