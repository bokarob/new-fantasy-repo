...
function Invoke-CurlRequest {
    ...
    $headerFile = Join-Path $env:TEMP ("player-stats-h-" + [guid]::NewGuid().ToString() + ".txt")
    $bodyFile = Join-Path $env:TEMP ("player-stats-b-" + [guid]::NewGuid().ToString() + ".txt")
    $jsonFile = $null
    ...
    try {
        ...
        if ($null -ne $JsonBody) {
            $jsonFile = Join-Path $env:TEMP ("player-stats-j-" + [guid]::NewGuid().ToString() + ".json")
...
Write-Host "Player stats smoke checks for TASK-022"
...
if (-not $token) {
    $runEmail = ("pstats_smoke_" + [int][double]::Parse((Get-Date -UFormat %s)) + "@example.com")
...
Write-Host "2) GET player stats -> expect 200 + ETag; then If-None-Match -> 304"
...
$s1 = Invoke-CurlRequest -Method GET -Url "$BaseUrl/leagues/$leagueId/stats/players?limit=20&offset=0" -Headers @("Authorization: Bearer $token")
$cc1 = Header-Value -Headers $s1.headers -Name "Cache-Control"
$etag1 = Header-Value -Headers $s1.headers -Name "ETag"
$okShape = $false
$okMetaEtag = $false
if ($s1.status -eq 200) {
    try {
        $obj = $s1.body | ConvertFrom-Json
        $okShape = ($obj.data.items -is [System.Array]) -and ($null -ne $obj.data.total)
        $okMetaEtag = [string]$obj.meta.etag -eq [string]$etag1
    } catch {}
}
if ($s1.status -eq 200 -and $cc1 -eq "private, must-revalidate" -and $etag1 -and $okShape -and $okMetaEtag) {
    Write-Host "PASS: player stats returned Category A response."
} else {
    Write-Host "FAIL: expected 200 + ETag + valid payload."
}
Write-Host "Status: $($s1.status)"
Write-Host "Cache-Control: $cc1"
Write-Host "ETag: $etag1"
Write-Host "Body: $($s1.body)"
Write-Host ""

if ($etag1) {
    $s304 = Invoke-CurlRequest -Method GET -Url "$BaseUrl/leagues/$leagueId/stats/players?limit=20&offset=0" -Headers @(
        "Authorization: Bearer $token",
        "If-None-Match: $etag1"
    )
    if ($s304.status -eq 304) {
        Write-Host "PASS: conditional request returned 304."
    } else {
        Write-Host "FAIL: expected 304 Not Modified."
    }
    Write-Host "Status: $($s304.status)"
    Write-Host "Body length: $($s304.body.Length)"
    Write-Host ""
}

Write-Host "3) Invalid sort -> expect 400 BAD_REQUEST"
$badSort = Invoke-CurlRequest -Method GET -Url "$BaseUrl/leagues/$leagueId/stats/players?sort=nope" -Headers @("Authorization: Bearer $token")
...
$invalid = Invoke-CurlRequest -Method GET -Url "$BaseUrl/leagues/$InvalidLeagueId/stats/players" -Headers @("Authorization: Bearer $token")
...
$noToken = Invoke-CurlRequest -Method GET -Url "$BaseUrl/leagues/$leagueId/stats/players"
