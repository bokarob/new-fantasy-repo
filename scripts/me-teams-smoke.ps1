...
function Invoke-CurlRequest {
    ...
    $headerFile = Join-Path $env:TEMP ("me-teams-smoke-h-" + [guid]::NewGuid().ToString() + ".txt")
    $bodyFile = Join-Path $env:TEMP ("me-teams-smoke-b-" + [guid]::NewGuid().ToString() + ".txt")
    $jsonFile = $null
    ...
    try {
        ...
        if ($null -ne $JsonBody) {
            $jsonFile = Join-Path $env:TEMP ("me-teams-smoke-j-" + [guid]::NewGuid().ToString() + ".json")
...
Write-Host "Me Teams endpoint smoke checks for TASK-013"
...
if (-not $token) {
    $runEmail = ("me_teams_smoke_" + [int][double]::Parse((Get-Date -UFormat %s)) + "@example.com")
...
Write-Host "2) GET /me/teams -> expect 200 + Category A headers"
$teams1 = Invoke-CurlRequest -Method GET -Url "$BaseUrl/me/teams" -Headers @("Authorization: Bearer $token")
$cc1 = Header-Value -Headers $teams1.headers -Name "Cache-Control"
$etag1 = Header-Value -Headers $teams1.headers -Name "ETag"
$hasTeamsArray = $false
$metaEtagMatches = $false
if ($teams1.status -eq 200) {
    try {
        $obj = $teams1.body | ConvertFrom-Json
        $hasTeamsArray = $obj.data.teams -is [array]
        $metaEtagMatches = $obj.meta.etag -eq $etag1
    } catch {}
}
if ($teams1.status -eq 200 -and $cc1 -eq "private, must-revalidate" -and $etag1 -and $hasTeamsArray -and $metaEtagMatches) {
    Write-Host "PASS: /me/teams returned 200 with expected envelope and cache headers."
} else {
    Write-Host "FAIL: /me/teams did not meet expected response/header requirements."
}
Write-Host "Status: $($teams1.status)"
Write-Host "Cache-Control: $cc1"
Write-Host "ETag: $etag1"
Write-Host "Body: $($teams1.body)"
Write-Host ""

if ($etag1) {
    Write-Host "3) Repeat with If-None-Match -> expect 304"
    $teams304 = Invoke-CurlRequest -Method GET -Url "$BaseUrl/me/teams" -Headers @(
        "Authorization: Bearer $token",
        "If-None-Match: $etag1"
    )
    if ($teams304.status -eq 304) {
        Write-Host "PASS: conditional request returned 304 Not Modified."
    } else {
        Write-Host "FAIL: expected 304 Not Modified."
    }
    $etag304 = Header-Value -Headers $teams304.headers -Name "ETag"
    Write-Host "If-None-Match sent: $etag1"
    Write-Host "Status: $($teams304.status)"
    Write-Host "ETag returned: $etag304"
    Write-Host "Body length: $($teams304.body.Length)"
    Write-Host ""
}

Write-Host "4) No token -> expect 401 AUTH_REQUIRED"
$noToken = Invoke-CurlRequest -Method GET -Url "$BaseUrl/me/teams"
