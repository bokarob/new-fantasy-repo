...
| GET /leagues/{league_id}/transfers | Payload | Transfer history (My Transfers), Team management drilldown | A | user+league(+query) scoped: max(transfer_id)+count for competitor filter + current_gw usage markers | after transfer confirm |
